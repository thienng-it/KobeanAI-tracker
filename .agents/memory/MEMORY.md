...
Model resolution and filtering must strictly use ModelRegistry and support granular model specs and per-model statistics isolation across all dashboard routes and React components.
...
Gitleaks CI scans for database URIs with credentials. Never write mock connection strings containing plaintext auth tokens in URLs. Use 'postgresql://localhost:5432/mydb' or process.env.DATABASE_URL environment variables.
